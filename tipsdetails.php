...
<!--Main Navigation-->
    ...
    <?php 
          $tipsid=$_GET['tipsid'] ;
           $blog=mysqli_query($db,"SELECT * FROM guideline  WHERE guideline_id = $tipsid ");
    ...
    <div class="card-header dark d-flex justify-content-between align-items-center py-3" style="background-color: rgb(35 35 35)">
        <div class="d-flex justify-content-start align-items-center">
          <div class="d-flex flex-column pl-3">
            ...
            <p class="small text-muted mb-0">Posted Time: <?php echo $time;?></p>
          </div>
        </div>
      </div>
      <div class="card-body">
        <img src="tipsimage/<?php echo $img;?>" width="300px" class="img-fluid">
      ...
      <?php 
          $tipsid=$_GET['tipsid'] ;
           $blog=mysqli_query($db,"SELECT * FROM gl_comment  WHERE gl_id = $tipsid ");

           if (mysqli_num_rows($blog) > 0) {
            // output data of each row
            while($get = mysqli_fetch_assoc($blog)) {    
           $comment = $get['comment'];
           $commentid = $get['gl_comment_id'];
      ...
      <div class="py-4 grey lighten-4">
        ...
        $image = $get['image'];
        if ($uid_form_comment == $_SESSION['login_user']){
        $name = "You";
        }
        else {
      ...
      <div class="card-footer white py-3">
        <div class="form-group mb-0">
          ...
          <div class="text-right pt-1">
            <button type="submit" name="gl_comment_submit" class="btn btn-primary btn-sm mb-0 mr-0">Submit</button>
          </div>
          </form>
        </div>
      </div>
